...
- Forgetting to update pybind11 wrappers (`src/*.cpp`) when C++ signatures change in `include/svs/`.
- Not testing edge cases: empty arrays, mismatched dtypes, out-of-bounds indices.
- Returning C++ raw pointers without proper lifetime management → use `py::return_value_policy`.
