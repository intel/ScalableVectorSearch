...
const char* HELP = R"(
Run a search-only benchmark for the IVF index.
...
1. Run all the benchmarks in the global `search_ivf_static` array in `src-file.toml`.
   All elements in the array must be parseable as a ``svsbenchmark::ivf::SearchJob``.
...
struct Exe {
  public:
    using job_type = ivf::SearchJob;
    using dispatcher_type = ivf::StaticSearchDispatcher;

    static dispatcher_type dispatcher() {
        auto dispatcher = dispatcher_type{};
        ivf::register_uncompressed_static_search(dispatcher);
        return dispatcher;
    }
