...
// Tag for dispatching when ingesting loaders into index constructors.
// Essentially promises that a class with this tag type will return some kind of simple
// data when loaded.
struct VectorDataLoaderTag : public lib::AbstractLoaderTag {};

// Forward Declaration
template <typename T, size_t Dims, typename Builder> class VectorDataLoader;
...
template <typename Allocator = HugepageAllocator> struct UnspecializedVectorDataLoader {
    ...
    // Use a default dataset type of ``data::SimplePolymorphicData`` to abstract away
    // allocators.
    //
    // If the returned type should be different, then an overload is provided that accepts
    // a data builder.
    template <typename T, size_t Dims = Dynamic>
    VectorDataLoader<T, Dims, data::PolymorphicBuilder<Allocator>> refine(
        meta::Type<T> SVS_UNUSED(type), meta::Val<Dims> SVS_UNUSED(dims) = meta::Val<Dims>()
    ) const {
        return VectorDataLoader<T, Dims, data::PolymorphicBuilder<Allocator>>{
            *this, data::PolymorphicBuilder{allocator_}};
    }

    // Refinement using an externally supplied builder.
    template <typename T, size_t Dims, typename Builder>
    VectorDataLoader<T, Dims, Builder> refine(
        meta::Type<T> SVS_UNUSED(type),
        meta::Val<Dims> SVS_UNUSED(dims),
        const Builder& builder
    ) const {
        return VectorDataLoader<T, Dims, Builder>{*this, builder};
    }
...
class VectorDataLoader {
  public:
    ...
    using return_type = data::builder_return_type<Builder, T, Extent>;
    ...
    VectorDataLoader(lib::InferPath SVS_UNUSED(tag))
        : path_{std::nullopt} {}

    template <typename... Args>
    VectorDataLoader(lib::InferPath SVS_UNUSED(tag), Args&&... args)
    ...
    ///
    VectorDataLoader(const std::filesystem::path& path, Builder builder)
    ...
    VectorDataLoader(const UnspecializedVectorDataLoader<Allocator>& other, Builder builder)
        : VectorDataLoader{other.path_, std::move(builder)} {
    ...
    VectorDataLoader(const UnspecializedVectorDataLoader<Allocator>& other)
        : VectorDataLoader{other, Builder{other.allocator_}} {}

    return_type unsafe_load_direct() const {
        return io::auto_load<T, Extent>(*path_, builder_);
    }
    ...
    return_type load_from_table(
        ...
        return io::load_dataset<T, Extent>(binaryfile.value(), builder_);
    }
