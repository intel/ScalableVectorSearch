...
/// @brief Type-erased wrapper for the low-level IVF iterator.
class IVFIterator {
  private:
    struct Interface {
        virtual svs::index::ivf::IVFSearchParameters
    ...
    template <typename Index, typename QueryType> struct Implementation : Interface {
        ...
        // well.
        using type = svs::index::ivf::BatchIterator<Index, QueryType>;
  ...
  public:
    ...
    /// for the search. For IVF, this determines the initial buffer size before the first
    /// batch. The default of 0 means the buffer will be sized based on the first batch_size
    /// passed to next().
    ...
    [[nodiscard]] svs::index::ivf::IVFSearchParameters
