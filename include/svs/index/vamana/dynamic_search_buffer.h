...
template <typename Idx, typename Cmp = std::less<>> class MutableBuffer {
  ...
  private:
    ...
    uint32_t valid_capacity_ = 0;
    // Index of the best unvisited candidate.
    uint32_t best_unvisited_ = 0;
  ...
  public:
    ...
    explicit MutableBuffer(
        ...
        , target_valid_{lib::narrow<uint32_t>(config.get_search_window_size())}
        , valid_capacity_{lib::narrow<uint32_t>(config.get_total_capacity())}
    ...
    void change_maxsize(SearchBufferConfig config) {
        ...
        uint32_t target_valid_temp = lib::narrow<uint32_t>(config.get_search_window_size());
        uint32_t valid_capacity_temp = lib::narrow<uint32_t>(config.get_total_capacity());
    ...
    size_t insert_inner(value_type neighbor) {
        ...
        best_unvisited_ = std::min(best_unvisited_, lib::narrow_cast<uint32_t>(i));
        return i;
    }
