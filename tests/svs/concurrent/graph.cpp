...
// The concurrent index carries its own graph types (per-node sequence locks,
// per-node spin locks, atomic adjacency-list access, and the optional reverse-edge
// index). They shadow the same names in `svs::graphs`, which is left untouched.
namespace graphs = svs::index::vamana::concurrent::graphs;

CATCH_TEST_CASE("Concurrent Simple Graph", "[concurrent][graphs][simple]") {
    using Idx = uint32_t;
    size_t n_nodes = 10;
    const size_t max_degree = 5;

    auto graph = graphs::SimpleGraph<Idx>(n_nodes, max_degree);
