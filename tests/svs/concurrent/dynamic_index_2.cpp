...
// The concurrent index is a separate type from `cc::MutableVamanaIndex`;
...
// Internal linkage for the whole helper block. The upstream
// `tests/svs/index/vamana/dynamic_index_2.cpp`, which this file is a port of, declares
// several of the same helpers at global scope (`Report`, its stream operator); both files
// link into the same `tests` binary, so leaving these external would be an ODR violation.
namespace {

// Adds a sink to the process-global logger for the duration of a scope.
//
// The upstream test this file is ported from pushes a capturing sink onto the global logger
// and never removes it. The sink outlives the vector it captures by reference, so every
// later log statement that reaches the global logger is a use-after-free. It is latent
// because whether the freed memory still looks like a `std::vector` is a matter of heap
// luck; when the luck runs out it surfaces as a `std::bad_alloc` or a SIGSEGV inside a
// completely unrelated test that happens to log a warning. Scope the push instead.
...
CATCH_TEST_CASE(
    "Testing Concurrent Graph Index", "[concurrent][graph_index][dynamic_index]"
) {
    ...
    auto global_sink_guard = ScopedGlobalSink{svs::logging::get(), global_callback_sink};
    ...
    auto data_mutable = cc::SegmentedBlockedData<Eltype, N>(num_indices_to_add, N);
    ...
    auto reloaded = cc::auto_dynamic_assemble(
        tmp / "config",
        SVS_LAZY(ccg::SimpleBlockedGraph<uint32_t>::load(tmp / "graph")),
        SVS_LAZY(cc::SegmentedBlockedData<float>::load(tmp / "data")),
        svs::DistanceL2(),
        2
    );
    ...
    reloaded = cc::auto_dynamic_assemble(
        tmp / "config",
        SVS_LAZY(ccg::SimpleBlockedGraph<uint32_t>::load(tmp / "graph")),
        SVS_LAZY(cc::SegmentedBlockedData<float>::load(tmp / "data")),
        svs::DistanceL2(),
...
CATCH_TEST_CASE(
    "Concurrent MutableVamanaIndex Per-Index Logging Test", "[concurrent][logging]"
) {
...
CATCH_TEST_CASE(
    "Concurrent MutableVamanaIndex Default Logger Test", "[concurrent][logging]"
) {
...
CATCH_TEST_CASE(
    "Concurrent Vamana Index Default Parameters", "[concurrent][parameter][vamana]"
) {
    using Catch::Approx;
