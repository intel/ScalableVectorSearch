...
// The concurrent multi-label index is a separate type from
// `svs::index::vamana::MultiMutableVamanaIndex`; the pre-existing indexes are untouched.
namespace cc = svs::index::vamana::concurrent;
namespace ccg = svs::index::vamana::concurrent::graphs;

namespace {

// The concurrent index is built on grow-stable blocked storage. Copy a plain
// `SimpleData` into that type so the same source vectors can serve both as the
// reference index's dataset and as the concurrent index's.
template <typename Src> auto to_segmented(const Src& src) {
    using T = typename Src::element_type;
    auto dst = cc::SegmentedBlockedData<T, Src::extent>(src.size(), src.dimensions());
...
CATCH_TEMPLATE_TEST_CASE(
    "Concurrent multi-vector dynamic vamana index",
    "[long][concurrent][index][vamana][multi]",
    svs::DistanceL2,
    svs::DistanceIP,
    ...
    CATCH_SECTION("Insertion/Deletion in duplicated test datasets") {
        ...
        auto test_index = cc::MultiMutableVamanaIndex(
            build_parameters, to_segmented(data), test_indices, Distance(), num_threads
        );
    ...
    CATCH_SECTION("Save/Load") {
        ...
        // `svs::GraphLoader` / `svs::VectorDataLoader` produce the pre-existing graph
        // and dataset types. Reload into the concurrent stack's own types instead.
        auto test_index_2 = cc::auto_multi_dynamic_assemble(
            config_dir,
            svs::lib::Lazy([&]() {
                return ccg::SimpleBlockedGraph<uint32_t>::load(graph_dir);
            }),
            svs::lib::Lazy([&]() {
                return cc::SegmentedBlockedData<Eltype, N>::load(data_dir);
            }),
            Distance(),
...
CATCH_TEST_CASE(
    "Concurrent MultiMutableVamana Index Save and Load",
    "[concurrent][index][vamana][multi][saveload]"
) {
    using Eltype = float;
    ...
    auto index = cc::MultiMutableVamanaIndex(
        build_parameters, to_segmented(data), test_indices, Distance(), num_threads
    );
    ...
    CATCH_SECTION("Load MultiMutableVamana Index being serialized natively to stream") {
        ...
        index.save(stream);
        {
            ...
            using Data_t = cc::SegmentedBlockedData<Eltype, N>;
            using GraphType = ccg::SimpleBlockedGraph<uint32_t>;

            auto loaded = cc::auto_multi_dynamic_assemble(
                stream,
    ...
    CATCH_SECTION("Load MultiMutableVamana Index being serialized with intermediate files"
        ...
        svs::lib::DirectoryArchiver::pack(tempdir, stream);
        {
            ...
            auto loaded = cc::auto_multi_dynamic_assemble(
                config_dir,
                svs::lib::Lazy([&]() { return GraphType::load(graph_dir); }),
                svs::lib::Lazy([&]() { return Data_t::load(data_dir); }),
                Distance(),
                num_threads
            );
...
CATCH_TEST_CASE(
    "Concurrent MultiMutableVamana Index Replace External ID",
    "[concurrent][index][vamana][multi]"
) {
    ...
    auto index = cc::MultiMutableVamanaIndex(
        parameters, to_segmented(data), indices, Distance(), num_threads
    );
    ...
    CATCH_REQUIRE(
        index.replace_external_id(old_label, new_label) == cc::ReplaceExternalIdResult::Ok
    );
    ...
    CATCH_SECTION("Renaming a non-existent label reports OldIdMissing") {
        // `old_label` was already renamed away above, so it no longer exists either.
        CATCH_REQUIRE(
            index.replace_external_id(old_label, new_label + 1) ==
    ...
    CATCH_SECTION("Renaming onto an existing label reports NewIdExists") {
        const size_t other_label = 2;
        CATCH_REQUIRE(
            index.replace_external_id(new_label, other_label) ==
