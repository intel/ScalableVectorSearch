...
template <> struct svs::index::vamana::concurrent::PruneStrategy<ThrowingL2> {
...
// `test_dataset::graph()` / `test_dataset::data_f32()` return the pre-existing graph and
...
auto concurrent_data_loader() {
    return svs::lib::Lazy([]() {
        return ConcurrentData::load(test_dataset::data_svs_file());
    });
}

// The concurrent analogue of `test_dataset::vamana::load_dynamic_test_index`.
template <typename Distance> auto load_concurrent_dynamic_test_index(const Distance& d) {
    return cc::auto_dynamic_assemble(
        test_dataset::vamana_config_file(),
        concurrent_graph_loader(),
        concurrent_data_loader(),
        d,
        1,
...
CATCH_TEST_CASE("Concurrent Vamana Iterator", "[concurrent][index][vamana][iterator]") {
    ...
    // No "Static Index" section: this stack adds a concurrent *dynamic* index only, and
    // leaves `svs::index::vamana::VamanaIndex` untouched (it keeps its own test).

    // For the dynamic index, iterated search should honor the internal deleted state of
    // IDs.
    CATCH_SECTION("Dynamic Index") {
        auto index = load_concurrent_dynamic_test_index(ThrowingL2());
    ...
    CATCH_SECTION("Multi batch iterator") {
        auto index = cc::auto_multi_dynamic_assemble(
            test_dataset::vamana_config_file(),
            concurrent_graph_loader(),
            concurrent_data_loader(),
            ThrowingL2(),
            1,
            cc::MultiMutableVamanaLoad::FROM_STATIC
        );
        auto original = test_dataset::data_f32();
        index.set_threadpool(svs::threads::DefaultThreadPool(2));
        auto itr = svs::threads::UnitRange{0, index.size()};
