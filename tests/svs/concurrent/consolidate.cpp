...
namespace graphs = svs::index::vamana::concurrent::graphs;
...
CATCH_TEST_CASE("Concurrent Graph Consolidation", "[concurrent][graph_index]") {
    // The concurrent consolidate requires the concurrent graph type: its
    // `add_edge` returns `AddEdgeResult` rather than a degree, and it carries the
    // per-node sequence locks the lock-free readers validate against.
    auto graph = graphs::SimpleGraph<uint32_t>::load(test_dataset::graph_file());
    ...
    CATCH_SECTION("Remove Even Nodes") {
        ...
        svs::index::vamana::concurrent::consolidate(
