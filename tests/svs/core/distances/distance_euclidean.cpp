...
double euclidean_reference(const std::vector<Ea>& a, const std::vector<Eb>& b) {
    assert(a.size() == b.size());
    double result = 0;
    for (size_t i = 0; i < a.size(); ++i) {
        auto x = svs_test::promote(a[i]) - svs_test::promote(b[i]);
        result += x * x;
    }
    return result;
}
...
void test_types(T lo, T hi, size_t num_tests) {
    ...
    for (size_t i = 0; i < num_tests; ++i) {
        ...
        CATCH_REQUIRE((svs::distance::L2::compute<N>(a.data(), b.data()) == expected));
        // Dynamically Sized Computation
        CATCH_REQUIRE((svs::distance::L2::compute(a.data(), b.data(), N) == expected));
    }
}
} // namespace
...
CATCH_TEST_CASE("Testing Euclidean Distance", "[distance][euclidean_distance]") {
    ...
    CATCH_SECTION("Saving and Loading") {
        ...
        auto x = svs::distance::DistanceL2{};
...
void run_benchmark(size_t num_elements, T lo, T hi, size_t ndims = N) {
    ...
    auto generator_a = svs_test::make_generator<Ea>(lo, hi, 0xcafe);
    svs_test::populate(a, generator_a, ndims);

    auto generator_b = svs_test::make_generator<Eb>(lo, hi, 0xbeef);
    std::vector<double> reference_distances{};
    for (size_t i = 0; i < svs::getsize<0>(data); ++i) {
        svs_test::populate(b, generator_b, ndims);
        reference_distances.emplace_back(euclidean_reference(a, b));
    ...
    NAMED_TEMPLATE_BENCHMARK("EuclideanDistance", (Ea, Eb, svs_test::Val<N>), ndims) {
    ...
    bool check = std::equal(
        ...
        [](const float& x, const float& y) { return x == Catch::Approx(y); }
    );
...
CATCH_TEST_CASE(
    ...
    "[distance][euclidean_distance][benchmark_suite][!benchmark]"
) {
    auto num_elements = 1000000;
    // Types: `float` and `float`
    run_benchmark<float, float, 128>(num_elements, -100.0f, 100.0f);
    run_benchmark<float, float, 0>(num_elements, -100.0f, 100.0f, 128);
    run_benchmark<float, float, 100>(num_elements, -100.0f, 100.0f);
    run_benchmark<float, float, 0>(num_elements, -100.0f, 100.0f, 100);

    // Types: `float` and `svs::Float16`
    run_benchmark<float, svs::Float16, 128>(num_elements, -100.0f, 100.0f);
    run_benchmark<float, svs::Float16, 0>(num_elements, -100.0f, 100.0f, 128);
    run_benchmark<float, svs::Float16, 100>(num_elements, -100.0f, 100.0f);
    run_benchmark<float, svs::Float16, 0>(num_elements, -100.0f, 100.0f, 100);

    // Types: `svs::Float16` and `svs::Float16`
    run_benchmark<svs::Float16, svs::Float16, 128>(num_elements, -100.0f, 100.0f);
    run_benchmark<svs::Float16, svs::Float16, 0>(num_elements, -100.0f, 100.0f, 128);
    run_benchmark<svs::Float16, svs::Float16, 100>(num_elements, -100.0f, 100.0f);
    run_benchmark<svs::Float16, svs::Float16, 0>(num_elements, -100.0f, 100.0f, 100);
