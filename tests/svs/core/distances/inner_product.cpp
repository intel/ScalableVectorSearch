...
void test_types(T lo, T hi, size_t num_tests) {
    ...
    for (size_t i = 0; i < num_tests; ++i) {
        ...
        for (auto arch : supported_archs) {
            ...
            auto dist = svs::distance::DistanceIP{};
        ...
        // are already tested above.
        {
            ...
            auto dist = svs::distance::IP<svs::arch::MicroArch::baseline>::compute<N>(
                a.data(), b.data()
            );
        ...
        }
        {
            ...
            auto dist = svs::distance::IP<svs::arch::MicroArch::baseline>::compute(
                a.data(), b.data(), N
            );
            CATCH_REQUIRE(dist == expected);
        }
